...
> **大小写不靠 OCR 定，靠"行内字高"量。** 那行页脚源图是 `O.V.D.`，
> OCR 交出来 `O.v.D.` —— 而且 **6 个二值化变体全错**（二值化削掉了 V 顶端的
> 细笔画），多数投票和置信度都救不了。脚本对 `c o s v w x z` 这六个上下同形的
> 字母额外做一次几何裁决：行内用无升降部小写字母定 x-height、用大写字母定
> cap-height，把待判字母归到更近的一边。实测该字高 12px、本行 x-height 9px /
> cap-height 12px → 判为大写。日志里会写依据 `v->V(高12: x9/cap12)`。
> 详见 `docs/图片与CDR自动重绘使用教程.md` 的 Q25。
...
整页影响很小（页脚只占 371×16 px / 1217×1660）：**94.65% → 94.29%**，
即用 0.36 个百分点的整页还原度换这一行字可编辑。
...
**⚠️ 保存必须核验落盘，不能只看"没报错"。** 这一条是真事故：整轮跑完、
内存里回读核验全对、日志打印"已保存"，重新打开文件发现改动**根本没进去**，
白跑一轮还差点当成交付完成。根因是目标 CDR 被另一个进程打开（**用户自己开着
CorelDRAW 在看这个文件**），文件处于只读状态，而 `doc.Save()` 在这种情况下
**既不抛异常、也不写文件**：
...
修法：保存前后各取一次 `(大小, mtime_ns)` 指纹，没变就报错并以退出码 3 结束。
...
覆盖五块：纯逻辑（提示词渲染、CDR 结构比对）；**用一张几何已知的合成图**
验证自动分区的每一处坑——残留剥离、外沿外扩、满版色带贯通判据、行中位数、
列方向切分、超采样度量的偏差量级；文字转活字的判定（连通分量、四条判据的边界）；
OCR 的预处理与变体选择（Otsu 平台期、二值化极性、变体选择规则）；
保存落盘核验的文件指纹。改坏了立刻报出来。
