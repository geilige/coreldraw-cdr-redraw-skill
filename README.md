...
- **位图矢量化是近似还原**，不等于原始矢量。实测还原度（210×297mm 包装封面，
  源图 1217×1660，按源图原生分辨率比对）：
  反白大字号 ≈ 99.5%、精细插图 ≈ 97.5%、Fragile 标 ≈ 98.3%、
  6pt 级小字 ≈ 93.8%；整页配准 IoU ≈ 93.5%。
  笔画粗细（墨迹面积比）偏差 ≤ 0.7%。交付时会给出配准指标并说明哪些是描摹近似。
- **判断还原度不要靠眼睛**。源图原生密度与 CorelDRAW 导出密度不同，
  直接并排对照会让重绘显得又大又粗。要算「墨迹面积比」（与分辨率无关），
  并把对照图重采样到同一 px/mm。详见
  [`raster-to-vector-notes.md`](skills/coreldraw-x8-redraw/references/raster-to-vector-notes.md) 的 5.7 / 5.8。
