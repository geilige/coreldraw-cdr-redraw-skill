...
### ⚠️ 每次属性访问都是一次跨进程调用——先估算调用次数

COM 单次调用是**毫秒级**的，写循环之前先算一下会跑多少次。真踩过：

> 想核验"转活字只动了页脚"，第一版写的是**逐节点**比对
> （`SubPaths.Item(i).Segments.Item(j).StartNode.PositionX`…）。
> `04_ART` 那张插图有 **483 子路径 / 12328 段**，逐节点要几万次调用，
> **跑满 4 分钟还卡着**。
> 改成只读「类型 + 包围盒 + 子路径数 + 段数」后（每形状约 `1 + 子路径数` 次调用），
> **秒过**。
...
| 写法 | 调用次数量级 | 12k 段的图形 |
| --- | --- | --- |
| `Shape.Type` / `SizeWidth` | 1 | 快 |
| `Curve.SubPaths.Count` | 1 | 快 |
| `SubPaths.Item(i).Segments.Count` | 1 + 子路径数 | 约 500 次，可接受 |
| 逐段读 `.Type` | 1 + 子路径数 + 段数 | **约 1.3 万次，慢到不可用** |
| 逐节点读 `.PositionX/.Y` | × 2 再乘一遍 | **约 2.6 万次，卡死** |

**要省就得省到底**：只把「逐段」换成「只数段数」、却仍逐段读 `.Type`，
等于一点没省——这个坑我第二次还踩了一遍。

**需要逐节点等价比对时**，先想清楚是不是真有必要：CDR 里做这件事通常要先
`ConvertToCurves`，那会**改动文档**，只读核验不能碰。多数场景用
「类型 + 包围盒 + 计数」这组廉价指纹就足够定位问题。

### 被强杀的脚本会留下锁

脚本被 kill 时，CorelDRAW 实例里打开的文档**不会自动关闭**，文件就被锁住
（表现：别的程序写它报 `Permission denied`）。收尾时连上实例、
**先读 `doc.Dirty`，为 False 才 `Close()`**（为 True 说明有未保存内容，别动）。
