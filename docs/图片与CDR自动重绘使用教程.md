# CDR 自动重绘使用教程

本教程面向第一次使用 `coreldraw-x8-redraw` 技能的人，从环境准备到跑出第一个文件。

## 这个技能的主用途

**给一张位图，自动操作 CorelDRAW 把它画成矢量图。**

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-width 210 --output out\ref.cdr
```

只用给**源图**和**页面宽度**，脚本自动完成六步（标定 → 边缘残留探测 → 自动分区 →
逐区寻优描摹 → 在 CorelDRAW 中重建 → 配准式校验），产出 CDR、预览 PNG、
描摹 SVG 与清单、定位记录、差异对照图、还原度报告。详见**第二节**。

如果你手上有现成的 `.cdr` / `.pdf` / 图纸尺寸，技能也支持：

| 章节 | 模式 | 适用场景 |
| --- | --- | --- |
| **二** | **位图矢量化（主用途）** | **只有位图，要"照着画进 CDR"** |
| 三 | 源 CDR | 手上有原始 `.cdr` |
| 四 | 图片 + 尺寸 | 截图/草图 + 书面尺寸 |
| 五 | PDF 派生 | 只有 PDF，没有源 CDR |

> 第二节是踩坑最多的一节。它对应主入口 `cdr_bitmap_to_cdr.py`、
> 三个子脚本（`cdr_image_trace.py` / `cdr_image_place.py` / `cdr_visual_diff.py`）
> 和一份实战笔记 `references/raster-to-vector-notes.md`。

---

## 一、准备环境

### 1. 安装官方完整版 CorelDRAW X8 或更高

这是**硬性前提**。COM 自动化依赖 CorelDRAW 在安装时写入的注册表项与类型库。

> ⚠️ 第三方"精简版 / 绿色版 / 便携版"通常删掉了 COM 注册脚本与 VBA 宿主组件。
> 用它们会直接报：
>
> ```
> Invalid class string
> ActiveX component can't create object
> ```
>
> 遇到这两个错误，第一反应就应该是"我装的是不是精简版"。

### 2. 安装 Python 与依赖

```powershell
python --version          # 需要 3.10 或更高
python -m pip install pywin32
```

如果要用**位图矢量化模式**（第六节），再装四个：

```powershell
python -m pip install numpy opencv-python pillow potracer
```

### 3. 验证 COM 通路

```powershell
python -c "import win32com.client as w; app=w.Dispatch('CorelDRAW.Application'); app.Visible=True; print('CorelDRAW 版本', app.VersionMajor, app.VersionMinor)"
```

能打印出版本号（X8 应输出 `18`）就说明通路正常。

### 4. 安装技能

把 `skills/coreldraw-x8-redraw` 复制到你的技能目录：

```powershell
xcopy /E /I skills\coreldraw-x8-redraw "%USERPROFILE%\.workbuddy-ai\skills\coreldraw-x8-redraw"
```

然后**重启客户端**。

### 5. 离线回归测试（可选）

没装 CorelDRAW 也能验证技能自带的逻辑是否正确（196 项断言）：

```powershell
python skills\coreldraw-x8-redraw\scripts\selftest_offline.py
```

应输出 `全部测试通过。`

它除了纯逻辑（提示词渲染、CDR 结构比对），还用一张**几何已知的合成图**验证
自动分区的每一处坑：残留剥离、外沿外扩、满版色带贯通判据、行中位数、
列方向切分、超采样度量的偏差量级。合成图刻意做成"左侧 4 列 + 顶部 3 行贯穿残留、
满版色带被反白字掏空、同一行两个实心块、细笔画文字带浅灰抗锯齿外沿"，
每处都对应一个真实踩过的坑。

---

## 二、第一个重绘：源 CDR 模式

### 场景

你有一个 `包装设计稿.cdr`，想生成一份结构完全对齐的新文件。

### 步骤 1：剖析并生成定制提示词

对 AI 说：

```
使用 coreldraw-x8-redraw。源文件 D:\work\包装设计稿.cdr。
先生成定制重绘提示词，输出到 D:\work\outputs\。
```

技能会调用：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_prompt_builder.py ^
  --source "D:\work\包装设计稿.cdr" ^
  --output "D:\work\outputs\包装设计稿-redraw-prompt.md"
```

### 步骤 2：审阅提示词

打开生成的 `包装设计稿-redraw-prompt.md`，重点看这几节：

| 章节 | 看什么 |
| --- | --- |
| 1. 文件指纹 | 页面数、形状总数、单位是否符合预期 |
| 4. 页面清单 | 页面尺寸与方向是否正确 |
| 5. 图层清单 | 图层是否齐全，有没有该锁定的图层 |
| 6. 形状类型分布 | 文本 / 位图 / 曲线数量是否合理 |
| 7. 文本清单 | 有没有乱码或空文本 |
| 10. 已知风险 | 有没有需要特别注意的点 |

**这一步很重要。** 如果指纹就已经不对（比如页面数少了），说明源文件本身有问题，
先解决它，别急着往下跑。

### 步骤 3：执行重绘

```
确认无误，执行精确重绘，输出到 D:\work\outputs\。
```

技能会调用：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_redraw.py ^
  --source "D:\work\包装设计稿.cdr" ^
  --output "D:\work\outputs\包装设计稿_redraw_exact.cdr" ^
  --mode clone
```

### 步骤 4：看校验结果

输出末尾会打印：

```
============================================================
源文件：页面 2，形状 37
目标文件：页面 2，形状 37
校验结果：PASS（结构一致）
```

- **PASS**：页面数、图层名、顶层形状数、形状总数、类型分布全部一致。
- **CHECK WARNINGS**：会逐条列出差异，例如：

```
校验结果：CHECK WARNINGS（存在差异）
  - 图层 图层 1 形状总数不一致：源 18 / 目标 15
  - 全文档形状类型分布不一致：
      TextShape(文本)[6]：源 10 / 目标 7
```

出现警告时，先看是**哪一类形状**少了。如果是文本少了，通常是字体缺失导致的复制失败；
如果是效果类形状少了，通常是阴影 / 立体化在跨文档复制时被简化。

---

## 三、保底方案：文件级复制

如果 `--mode clone` 在你的 CorelDRAW 版本上失败率高（`CopyToLayer` 跨文档不支持），
用文件级复制：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_redraw.py ^
  --source "D:\work\包装设计稿.cdr" ^
  --output "D:\work\outputs\包装设计稿_copy.cdr" ^
  --mode duplicate
```

这个模式直接把源文件复制一份，内容 **100% 一致**，然后再用 COM 打开做结构校验。
它不产出"重建后的可编辑新结构"，但作为回归基线和保底交付非常可靠。

---

## 四、图片 + 尺寸模式

### 场景

客户只给了一张 JPG 效果图，外加几个关键尺寸。

### 怎么提问

```
使用 coreldraw-x8-redraw 的图片+尺寸模式：
- 参考图：D:\ref\效果图.jpg
- 成品尺寸：210 × 297 mm
- 单位：毫米
- 关键尺寸：主标题距上边缘 40mm，Logo 宽 60mm
- 输出：D:\out\redraw.cdr
冲突时以我给的尺寸为准。
```

### 规则

技能会按这个优先级解析几何：

```
书面尺寸  >  派生算术  >  图片比例
```

图片只用来判断**拓扑、顺序、视觉关系**，绝不用来替代数值尺寸。

### 结果定性

因为源是图片，技能会把产出标注为**近似重建**，不会声称与某个 CDR 完全一致。
只有当你提供了足够权威的尺寸与单位时，才会做尺寸驱动的重建。

---

## 五、PDF 派生模式

只有 PDF、没有源 CDR 时：

1. 技能先判断 PDF 的构成：可提取矢量路径 / 嵌入位图 / 可提取文本 / 混合。
2. 矢量 PDF 优先走矢量路径转换；栅格 PDF 高 DPI 渲染后当视觉证据。
3. 生成中间 CDR（**不覆盖**原 PDF）。
4. 再对中间 CDR 走常规重绘与校验流程。
5. 报告里会明确说明几何来自矢量路径、栅格描摹、OCR 还是推断。

> 中间 CDR **不是**原始 CDR 的精确副本，报告里不会这么写。

---

## 六、位图矢量化模式

### 场景

你只有一张图（截图、扫描件、设计稿导出图），要求"照着这张图在 CDR 里画出来"。

> **⚠️ 图纸里有文字时，先看「文字转活字」一节（本节下方）。**
> 默认的"一条命令"会把文字也描成轮廓——**看着像，但改不了字、换不了字体**。
> 有文字就必须走"先识别、后描摹"的四步流程。

### 怎么提问

```
使用 coreldraw-x8-redraw 的位图矢量化模式。
参考图 D:\ref.png，成品尺寸 210×297mm，单位毫米，输出 D:\out\cover.cdr。
严格参照图纸的布局与细节。
```

### 一条命令（推荐）

主入口把六步全串起来了，只用给源图和页面宽度：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-width 210 --output out\cover.cdr
```

产出：

| 产物 | 位置 |
| --- | --- |
| 矢量 CDR | `--output` 指定 |
| 预览 PNG | `<out-dir>\cover_preview.png` |
| 描摹 SVG 与清单 | `<out-dir>\svg\` |
| 定位记录 | CDR 同目录 `placement.json` |
| 差异对照图与指标 | `<out-dir>\compare\` |
| 报告 | `<out-dir>\report.md` |

`<out-dir>` 默认是 CDR 同目录下的 `cover_work`。

常用参数：

```powershell
# 用标准纸型（高度按纸型取，不按图比例推导）
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-size A4 --output out\cover.cdr

# 没有 CorelDRAW 也能跑：只描摹出 SVG 与清单
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-width 210 --output out\cover.cdr --trace-only

# 裁掉左侧扫描残留后重新标定
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-width 210 --output out\cover.cdr --crop-left 4

# 自动分区不满意时手工指定（覆盖自动结果）
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --image ref.png --page-width 210 --output out\cover.cdr ^
  --region "art_tools:6,500,1217,900" ^
  --region "logo_vonder:0,901,1217,1059,invert"

# 只重建报告，不重跑描摹与建 CDR
python skills\coreldraw-x8-redraw\scripts\cdr_bitmap_to_cdr.py ^
  --report-only --out-dir out\cover_work --output out\cover.cdr
```

区域坐标是**源图像素**，左上为原点，格式 `名称:x0,y0,x1,y1[,invert]`。
`invert` 用于**深底上的白字/白图**。

### 分步跑（需要精细控制时）

主入口内部调用三个子脚本，也可以单独用。

**① 描摹** —— 先跑参数寻优，再正式生成：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_image_trace.py ^
  --image ref.png --page-width 210 --out svg --probe

python skills\coreldraw-x8-redraw\scripts\cdr_image_trace.py ^
  --image ref.png --page-width 210 --out svg ^
  --region "art_tools:6,500,1217,900" ^
  --region "logo_vonder:0,901,1217,1059,invert" ^
  --region "mark_fragile:876,338,1137,447" ^
  --region "text_footer:440,1075,870,1106"
```

**② 重建** —— 按清单在 CorelDRAW 里建页、建图层、导入定位：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_image_place.py ^
  --manifest svg\manifest.json ^
  --output out\cover.cdr --preview out\cover_preview.png ^
  --rect "band:0,155.472,210,27.264,#111111" ^
  --rect-layer BAND --layer-order "BAND,MARK,ART,TEXT" ^
  --layer "logo_vonder=LOGO" --white logo_vonder
```

`--rect` 用来画满版色块（色带、底块）——这类元素**不要描摹**，直接用原生矢量矩形，
又准又小。`--white` 指定哪些区域要填白（深底上的反白元素）。
注意 `--rect-layer` 指定的名字**必须出现在 `--layer-order` 里**，否则矩形无处安放。

**③ 校验** —— 配准式像素比对：

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_visual_diff.py ^
  --source ref.png --render out\cover_preview.png ^
  --placement out\placement.json --out compare
```

输出整页与分区域的 IoU / 召回 / 精确，以及差异叠加图（深灰=一致、
红=漏画、蓝=多画）和放大对照图。

### 自动分区是怎么判的

主入口用这几个判据，每条都有坑：

**① 行投影切内容带** —— 行内墨迹像素数 ≥ 4 视为有效行，空白 < 6 行则合并。

**② 判定"满版色带"必须用行的中位数，不要用覆盖率。**
色带中间常被反白字标掏空，覆盖率会掉到 0.85 以下，用覆盖率判定会把整条色带
误判成普通内容区。实测同一张图：

| 判定信号 | 结果 |
| --- | --- |
| 行覆盖率 ≥ 0.85 | 碎成 `901..960` + `1037..1059` 两段（错） |
| **行中位数 < 128** | **`901..1059` 整条**（对） |
| 行 25 分位 < 128 | 多出 `661..784` 假色带（插图密集区，错） |

**③ 但中位数还不够，必须再加横向贯通校验。** 两个并排的实心内容块也能把行中位数
压到 128 以下（各占 25% 宽度时暗像素就过半了），于是整条内容行带被误判成色带，
输出变成"垫底矩形 + 反白描摹"——完全错。满版色带的定义特征是**横向贯通**
（底色顶到左右页边），所以要求它的行范围内**每一列都有墨迹**。
详见 Q18b。

**④ 贯穿边缘的扫描残留必须先剥离。** 残留条让每一行都含墨迹像素，行投影就
永远找不到空隙，分区会把本应分开的区块粘成一整块。实测左侧 4px 黑边条把
易碎标、插图、页脚粘成了一个 `y 0..901` 的大块。

**⑤ 区域外沿要用宽松墨迹图向外扩。** 细笔画的末梢会被 `min_ink_px` 当空白切掉，
详见 Q19。

色带会拆成两件事：垫底的原生矢量**矩形**（颜色自动取自源图暗像素中位数）
+ 同一框内的**反白描摹**。实测自动分区与手工调优的毫米包围盒一致到 0.1mm：

| 区域 | 自动分区（mm） | 手工调优（mm） |
| --- | --- | --- |
| 色带 | 0.000,155.472..210.000,182.736 | 0,155.472..210,182.736 |
| 插图 | 56.253,96.631..170.312,152.539 | 56.253,96.545..170.291,152.604 |
| 易碎标 | 152.539,59.532..194.988,75.924 | 152.474,59.596..195.074,75.924 |
| 页脚 | 81.274,186.878..145.292,189.121 | 81.252,186.921..145.249,189.703 |

自动分区给出的名称（`art01` / `mark01` / `text01`）只是按尺寸猜的**命名启发式**，
不代表真实语义。


### 还原度预期

按**源图原生分辨率**比对（210×297mm 包装封面，源图 1217×1660）：

| 元素类型 | 典型 IoU |
| --- | --- |
| 反白大字号字标 | ≈ 99.5% |
| Fragile 标（含小图标） | ≈ 98.3% |
| 精细线稿插图 | ≈ 97.5% |
| 6pt 级小字 | ≈ 93.8% |
| 整页配准 | ≈ 93.7% |

**达不到 100% 是正常的**——描摹必然在边缘产生 1px 级偏差。
重点是看指标偏低的是"漏画"（召回低）还是"多画/变粗"（精确低）。

全自动流水线（一条命令，脚本自己分区）实测与上表一致，甚至略好：

| 区域 | 自动选中阈值 | 原生分辨率 IoU | 面积比 |
| --- | --- | --- | --- |
| `band01_ink`（反白字标） | 138 | 99.51% | 1.000 |
| `mark01`（易碎标） | 128 | 98.47% | 1.006 |
| `art01`（工具群插图） | 128 | 97.62% | 1.015 |
| `text01`（6pt 页脚） | 118 | 93.59% | 0.998 |
| **整页配准** | — | **93.70%**（召回 95.54% / 精确 97.98%） | — |

自动分区判出的毫米包围盒与手工调优**一致到 0.1 mm**，选中的阈值也与手工细调吻合。
所以先用默认参数跑一条命令，只有明显不对的区域才需要 `--region` 手工接管。

### ⚠️ 别用眼睛判断笔画粗细

源图原生密度（如 5.795 px/mm）与 CorelDRAW 导出密度（如 11.81 px/mm）不同。
把两者裁同一毫米范围后并排，导出图像素数多一倍，**放大同样倍数后显示尺寸不同**，
重绘会显得又大又粗——这是对照图的缺陷，不是几何问题。

要判断粗细，算**墨迹面积比**（与分辨率无关）：

```
矢量几何面积（按 60 px/mm 光栅化数像素）÷ 源图墨迹面积
```

比值落在 0.98~1.02 就说明粗细吻合。本流水线实测最终为 **0.993~1.007**。

> 两个容易踩的测量坑：
> 1. 把导出图降采样到源图尺寸后再阈值化，会给细笔画补边——
>    实测能把 1.00 的面积比放大成 1.39。
> 2. 度量必须算在源图原生网格上。任何更高的网格都要上采样源图，
>    模糊后细笔画在固定阈值下缩水，同样会把重绘误判成偏粗。

### 文字转活字（图纸里有文字时，这一步要**前置**，不是收尾）

> **⚠️ 顺序很重要：先识别文字，再描摹图形。**
> 直接整张描摹会把文字也描成轮廓曲线——**看着像，但改不了字、换不了字体**，
> 等于把可编辑内容降级成死图形。
>
> ```
> 1. cdr_scan_text.py          整图识别：文字 / 符号 / 图形块 + 每块文字朝向
> 2. cdr_text_live.py          逐区判定 convert（转活字）/ keep_trace（保留描摹）
> 3. cdr_bitmap_to_cdr.py      描摹【非文字块 + keep_trace 的文字区】
> 4. cdr_text_live.py --apply  把 convert 的文字建成可编辑美术字
> ```
>
> **第 3 步的描摹范围必须包含 `keep_trace` 的文字区。** 漏掉它，品牌字标和
> 特殊符号会**整个从成品里消失**（实测：`daiion` 字标、`4#` 都没了——
> 它们被判 `keep_trace`，而描摹范围只写了"非文字块"）。
> 同理，`--scan-json`（把文字从描摹里挖掉）**只能喂 `convert` 的条目**，
> 喂全量会把 `keep_trace` 的字标一起挖掉。
>
> 判定为 `keep_trace` **不等于失败**：字库里没有接近的字体时，保留描摹轮廓
> 才是对的。实测 `daiion` 是定制品牌字标（首字母高度等于 x-height，扫遍字体库
> 无一款如此），强行套 Arial Bold（IoU 0.60）反而是降级。这种情况要在报告里
> **写明原因**，而不是当成"识别失败"。

#### 第一步：整图识别

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_scan_text.py ^
  --image ref.png --out-dir out\scan --mm-per-px 0.17256 ^
  --emit-live out\live_regions.txt --emit-trace out\trace_regions.txt
```

产出 `scan.json`（逐条：框、文本、朝向、置信度、是否可疑）与 `scan_preview.png`
（彩色框预览：蓝=正置文字、橙=180° 文字、红=图标、绿=实心块、黄=混合块）。
**先看这张预览图**——框错了后面全错，这一步花 10 秒能省一轮返工。

两份下游清单格式不同，**别手抄**（手抄必然不同步）：

| 清单 | 格式 | 喂给 |
| --- | --- | --- |
| `--emit-live` | `NAME=y0,y1,x0,x1[,#RRGGBB][,rot=180]`（**y 在前**） | `cdr_text_live.py --region` |
| `--emit-trace` | `name:x0,y0,x1,y1[,#RRGGBB]`（**x 在前、冒号分隔**） | `cdr_bitmap_to_cdr.py --region` |

它解决的几件事：

- **调色板自动提取**：抗锯齿像素是"背景色 → 墨色"的线性混合，按**方向**
  （不是欧氏最近色）聚类才拿得到精确墨色。用最近色会把黑字的灰边判给洋红。
- **文字朝向判定**：OCR 对长文本的方向不敏感，必须用**渲染字形的 IoU** 复核
  0°/180°。整排倒置的标签靠这个才读得出来。
- **文字框吸附**：OCR 给的是紧框，会切掉相连笔画（实测倒置字标的 `i` 点在
  y529..535，OCR 框只到 y531）。会把框扩到**与之相连**的墨迹边界。

#### 第二步以后

```powershell
python skills\coreldraw-x8-redraw\scripts\cdr_text_live.py ^
  --image ref.png --region footer=1083,1100,470,842 ^
  --mm-per-px 0.17256 --out-dir out\text ^
  --apply out\cover.cdr --replace-traced footer=05_TEXT
```

不加 `--apply` 只做识别与判定，不碰 CorelDRAW。

**⚠️ `--replace-traced` 不是可选项。** 描摹轮廓与活字在**同一位置**，叠加等于把
这一行字**加粗一遍**。实测同一页脚，区域 IoU / 墨迹比：

| 方案 | IoU | 墨迹比 |
| --- | --- | --- |
| 描摹轮廓 | 92.82% | 1.046 |
| 活字 | 70.50% | 1.141 |
| **两者叠加** | **77.70%** | **1.286（比源图粗 28.6%）** |

两头不讨好：既没保住描摹的 IoU，又丢了活字的"可编辑"。转活字的意义是**替代**描摹。
删除时机：只在活字建好、且定位误差 ≤ 0.05mm 之后删（先删后建的话，
一旦建字失败就两头落空）。整页保真度 94.65% → 94.29%（−0.36pp），
用 0.36pp 换回可编辑性，值。

#### 同一内容印多份时：跨实例投票

设计稿常把同一标签重复印多份。这时**单看一份分不出、合起来分得出**，
脚本会自动分组投票：

| 候选读法 | 票数 | 置信和 | 覆盖区域 |
| --- | --- | --- | --- |
| `www.daiion.com` ✅ | 12 | 10.191 | 4 / 4 |
| `wwww.dalion.com` ❌ | 3 | 2.424 | 1 / 4 |

单区域看时两者字形 IoU 只差 0.005（纯噪声），投票后才拉开。
组内还会共用同一款字体、统一判定（多数决，**平票取保守 `keep_trace`**）。

**大小写不是靠 OCR 定的，是靠"行内字高"量的。** 这一步很关键，因为 OCR 在
小字号二值图上会稳定地把大写读成小写（真踩过：页脚 `O.V.D.` 被读成 `O.v.D.`，
而且 6 个二值化变体**全错**、只有灰度变体读对——多数投票救不了）。

脚本对 `c o s v w x z` 这六个"上下同形"的字母额外做一次几何裁决：

1. 在**同一行内**，用无升降部小写字母（`m e a r n u …`）的高度的中位数定
   `x-height`，用大写字母（`O D`）的高度的中位数定 `cap-height`；
2. 把待判字母的高度归到更近的一边。实测那条页脚：x-height 9px、
   cap-height 12~13px，待判的字高 12px → 离 cap 0.5px、离 x 2.5px，判为大写。

为什么必须"行内相对"：早先用过一个**绝对**字高阈值，把 `Importadora` 的
`m/r/a/e/s` 全判成了大写（16px 的字里 x-height 与 cap 只差 3px），
IoU 从 0.606 掉到 0.36。行内相对比较天然规避这个问题。

安全约束是**宁可漏判、不可错判**：锚点样本不足 2 个、或 cap 与 x 的差小于
2px（这一行本身区分度不够）、或高度差不到 1px（测量噪声），一律不动。
每次纠正都会在日志里写明依据，例如：

```
形状修正建议: …; v->V(高12: x9/cap12)
逐词裁决: …; O.v.D.->O.V.D. ✅
```

看到 `高12: x9/cap12` 就说明是"量出来的"，不是猜的。详见 Q25。

**⚠️ 目标 CDR 不能被别的程序打开，否则保存会静默失效——见 Q24。**
脚本现在会**自动解除**这类占用（只关路径匹配的那一个文档，且
`Dirty=True` 时拒绝关闭），所以通常不需要你手动干预；但若你正在编辑那个文件，
会被明确拒绝而不是悄悄丢改动——见 Q27。

**字体是猜出来的，所以每一步都算分，分数不够就拒绝转换。** 看产物 `live_text.json`
里的 `verdict` 与 `reject_kind`：

| `verdict` | `reject_kind` | 含义 | 怎么办 |
| --- | --- | --- | --- |
| `convert` | — | 可以转 | 报告里写明字体与匹配分数，**注明是推断而非源文件信息** |
| `keep_trace` | `weak_match` | 是文字，但库里没有够接近的字体 | 保留描摹轮廓，列出最接近的 3 个候选让用户定 |
| `keep_trace` | `not_text` | **这块根本不是一行文字** | 去查区域切分是不是切错了 |

活字建在**单独的 `<区名>_LIVE` 图层**（如 `footer_LIVE`），与描摹轮廓分层。
万一字体猜错，整层删掉即可，不会污染描摹结果。

实测 vonder 页脚那行 6pt 文字：

| 项 | 结果 |
| --- | --- |
| 识别文本 | `O.V.D. Importadora e Distribuidora Ltda. • Curitiba - PR` |
| 匹配字体 | **Swis721 Cn BT / Bold**，逐词对齐 IoU 0.7235，领先候选集中位 1.68 倍 |
| 重建结果 | **1 个文本对象**（可编辑）；描摹轮廓是 73 子路径 / 840 段 |
| 定位误差 | 0.0000 mm |

匹配环节的四个关键点（详见 `references/live-text-design.md`）：

1. **OCR 要跑多套预处理投票**。实测同一张图，`2× + 20px 四周留白`出乱码、
   `2× + 纵向 0 留白`出正确答案——没有一套参数对所有图都稳。
2. **相似度不能用绝对 IoU 门槛**。16px 小字即使文本与字体都对，IoU 也只有 0.61；
   用 0.62 的绝对门槛会把完美匹配也拒掉。改用 `lift = 最佳 ÷ 候选中位`。
3. **但光有相似度不够，必须补"这是不是文字"的硬门槛**——见 Q23。
4. **易混符号直接量字形**（`•` 宽高比 1.25 / 墨密度 0.80，`-` 宽高比 4.00），
   大小写改动才走整体 IoU 逐词裁决——单个窄字形只占整行约 1% 面积，全局 IoU 分不出来。

### 关键注意

- **标定**：`mm_per_px = 页面宽度mm / 图片宽度px`。若图宽高比与页面宽高比不一致，
  说明源图裁切过，纵向内容会与页面高度有偏差，交付说明里会写清楚。
- **源图边缘残留**：扫描/裁切留下的贯穿全高黑边条**不会**被复刻为设计内容，
  但会在报告里标注，避免被误判为漏画。
- **小字号文字**描摹后是轮廓而非活字。需要可编辑文本就做上面的"转活字"一步——
  字体猜错比描摹失真更严重，所以默认不自动替换。
- 详细的踩坑记录见
  [`references/raster-to-vector-notes.md`](../skills/coreldraw-x8-redraw/references/raster-to-vector-notes.md)。

---

## 七、常见问题

### Q1：报 `Invalid class string`

装的是精简版 CorelDRAW。换官方完整版。

### Q2：报 `Call was rejected by callee`

CorelDRAW 忙。脚本会自动重试。若持续失败，关掉多余的 CorelDRAW 窗口再试。

### Q3：自动化跑到一半被我点了一下窗口就断了

自动化期间**不要操作 CorelDRAW 窗口**。脚本会设置
`Optimization = True` + `EventsEnabled = False` 来减少干扰，但手动点击仍会打断 COM 调用。

### Q4：校验结果里文本数量变少了

源文件用了本机没装的字体。装上对应字体后重跑。

### Q5：位图变成红叉 / 断链

源文件用的是**链接位图**。把链接的图片一起复制到新目录，或先在 CorelDRAW 里
把链接位图嵌入（`位图 → 嵌入`）后重跑。

### Q6：`CopyToLayer` 复制失败

不同 CorelDRAW 版本对跨文档复制的支持不同。用 `--mode duplicate` 保底。

### Q7：形状类型 / 单位数值和我查到的不一样

不同版本的 `cdrShapeType` / `cdrUnit` 枚举数值有差异。脚本的设计是：

- 优先从 CorelDRAW 类型库常量读取真实值；
- 取不到时回退到内置表；
- **报告里始终同时输出原始数值**（如 `TextShape(文本)[6]`），避免误读；
- 设置目标单位时**直接复制源文档的原始单位代码**，不做数字映射。

所以看到 `[6]` 这类方括号里的数字时，以它为准。

### Q8：重新打开 CDR 后，坐标数值大了一截（比如 210 变成 8.27）

`Document.Unit` **不随文件保存**。设成毫米并保存后，重新打开会回到英寸（1）。
几何数据本身是绝对单位、完全没受影响，只是读值单位变了。

读坐标前先设置一次：

```python
doc.Unit = 3      # cdrMillimeter
```

### Q9：`SaveAs` 没报错，但文件不在指定位置

`SaveAs` / `Export` 会按 **CorelDRAW 自己的工作目录**解析相对路径。
调用前把路径转成绝对路径：

```python
doc.SaveAs(os.path.abspath(out_path), None)
```

技能自带的脚本已做此处理；自己写代码时要注意。

### Q10：描摹出来的图形边缘是折线，圆角变成了斜切

`alphamax=0` 会让 potrace 输出**纯多边形**（不做任何平滑）。
注意它的像素 IoU 反而可能最高，只看 IoU 会选出最差的结果。

判断方法：数一下 SVG 里的曲线段与直线段：

```python
print(svg.count("C"), svg.count("L"))   # 曲线段为 0 就是多边形
```

调高 `--alphamax`（0.5~1.0）。

### Q11：小图标（箭头、酒杯之类）描摹后变形

细笔画在原图上只有 1–2 px，直接描摹必然失真。
**解法是描摹前先把灰度上采样 4 倍再二值化**（`--upscale 4`），
不是调 alphamax。实测能把这类图标从"明显失真"救回"形状正确"。

### Q12：位图矢量化后比对 IoU 只有十几个百分点

多半是把**内容包围盒**的导出图直接和**整页**源图比了。
导出图通常是内容区（例如 210×130mm），不是整页 A4。
必须按 `placement.json` 记录的内容包围盒把渲染图贴回整页白底再比——
`cdr_visual_diff.py` 已自动处理。

### Q13：源图边缘有一条黑边，需要画进 CDR 吗

那是扫描/裁切残留，**不要复刻**。`cdr_image_trace.py` 会自动探测并提示，
用 `--crop-left N` 排除即可。但要在交付说明里提一句，避免被误认为漏画。

### Q14：对照图里重绘明显比原图粗，怎么办

先别改参数——**多半是对照图本身有缺陷**。源图原生密度（如 5.795 px/mm）
与导出密度（如 11.81 px/mm）不同，裁同一毫米范围后并排，显示尺寸不一致。

正确做法：

1. 把两张图重采样到**同一 px/mm** 再并排；
2. 用**墨迹面积比**判断粗细，不要靠眼睛（见第六节的说明）。

如果面积比确实偏离 1.0 超过 2%，再调 `--threshold`
（越低越细）并重跑 `--probe`。

### Q15：小图标（酒杯、箭头之类）描摹后变形

见 Q11——提高 `--upscale`（默认已是 8），不要靠调 alphamax 解决。

### Q16：`--probe` 跑得太慢

上采样倍数越大越慢。大区域（整版插图）的 U=8 位图可能有数千万像素，
单次描摹就要一分钟以上。对策：

- 大区域用 `--upscale 4`，小区域用 8；
- 或者分区域分别跑 `--probe`，只扫关心的那个区域；
- 主入口有 `--max-tune-px`（默认 400000）：源像素面积超过它的区域只扫 3 个
  候选阈值而不是 5 个。

### Q17：自动分区把好几块内容粘成了一整块

九成是**源图边缘的贯穿残留**造成的。残留条让每一行都含墨迹像素，
行投影就永远找不到空隙。实测左侧 4px 黑边条把易碎标、插图、页脚粘成了一个
`y 0..901` 的大块。

主入口会自动把"整列/整行几乎全暗（占比 > 0.9）"的边条从投影判定里剔除，
日志里会打印 `已从投影中剔除贯穿边缘的残留：左 4 列…`。
如果残留不是贯穿全高（比如只在下半段），自动剔除可能失效，此时用
`--crop-left N` 手工裁掉后重新标定。

### Q18：逐区寻优选中的阈值总是最粗的那个

说明你用的是**硬边光栅化**在做评估。`cv2.fillPoly` 是"取整后填充"，
而且把多边形左右两个端点列**都**算作填充，于是每条扫描线多出 1 个像素——
渲染结果比真实几何**粗 1 个像素**。这个恒定加粗造成的偏差方向固定：
**笔画越粗 IoU 越高**（IoU = w/(w+1)）。

| 真实宽度 | 硬边渲染宽度 | 硬边 IoU% | 超采样渲染宽度 | 超采样 IoU% |
| --- | --- | --- | --- | --- |
| 1 | 2 | 50.00 | 1 | 100.00 |
| 3 | 4 | 75.00 | 3 | 100.00 |
| 10 | 11 | 90.91 | 10 | 100.00 |
| 40 | 41 | 97.56 | 40 | 100.00 |

真实数据上的表现（U=8）：

| 阈值 | 硬边 IoU% | 超采样 IoU% | 超采样面积比 |
| --- | --- | --- | --- |
| 128 | 94.15 | 99.40 | 1.005 |
| **138** | 94.29 | **99.51** | **1.000** |
| 148 | **94.34（硬边最高）** | 99.35 | 0.996 |

硬边把最优判成 148（过粗），超采样判成 138（正确）；而 CorelDRAW 实际导出
比对是 99.15%，说明超采样才贴近真实。

**修法**：评估时先按 8 倍超采样再盒式降采样得到覆盖率，以 `覆盖率 >= 0.5`
作为墨迹判据。技能内已按此实现。另外记得**按子路径自身包围盒分配小画布**
再异或回总画布，否则 483 子路径的区域在 ss=8 下要做 66 亿次像素操作。

> **残留偏差**：超采样把"每扫描线多 1 个超采样像素"折算成 +1/ss 个输出像素
> （ss=8 时 +0.125px）。方向仍是偏粗但已小到不影响阈值选择，
> 所以这个指标适合用来**选阈值**，不要当成精确面积。
>
> 另外这个偏差是 sub-pixel 效应。在远高于源图密度的网格上（如按 60 px/mm
> 数矢量面积算面积比）可以忽略，但在源图原生网格上算 IoU 就必须超采样。

### Q18b：两个并排的实心块被当成了"满版色带"

症状：两块内容被当成"垫底矩形 + 反白描摹"输出——矩形拿到两块的整体包围盒
并填成深色，块内内容反白。日志里能看到一个 `BAND` 区域横跨了本应分开的两块。

原因是**行中位数会被两个并排的实心块骗到**：中位数只问"这一行整体偏深吗"，
两块各占 25% 宽度时，暗像素就过半了，中位数低于 128。
实测合成图（600px 宽、两个 150px 实心黑块并排）中位数确实是暗的。

**修法**：加一条横向贯通判据——满版色带的定义特征是底色顶到左右页边，
所以它的行范围内**每一列都必须有墨迹**。两块并排内容之间的空隙列则一个
墨迹像素都没有，于是被正确判为内容。

```python
def _is_full_bleed(ink_raw, y0, y1):
    return bool(ink_raw[y0:y1, :].any(axis=0).all())
```

两个要点：用**未剥离**的墨迹图（残留条本身也是贯通的，不该让真色带被否掉）；
反白字标掏空不影响判定（字的上下仍有纯底色行，被掏空的列照样有墨迹）。
技能内已按此实现，实测在真实图上分区结果逐像素不变，是纯增强。

### Q19：文字下缘、图形末梢被切掉了

区域外沿不能只用严格墨迹图（gray<128）来定。细笔画的末梢（文字 descender、
抗锯齿边缘）每行只有两三个墨迹像素，会被 `min_ink_px` 当成空白切掉。
实测页脚墨迹真实范围 y 1083..**1098**，严格图只到 1096，切掉下缘后召回从
96.69% 掉到 86.65%。

技能内用**宽松墨迹图**（gray<200，含抗锯齿过渡像素）逐行/逐列向外扩，
**遇到真正的空白行/列就停**。

> 不要改成无脑 padding：padding 会越过区块间空白伸进相邻区域，
> 把邻块墨迹也描一遍，在 CDR 里产生重复几何。

### Q20：整页比对 IoU 只有九十几，剩下几个点差在哪

按这个顺序排查：

1. **源图边缘残留**没复刻（这是有意为之）。差异图里沿边缘的贯穿红线就是它。
2. **小字**天然失真。6pt 级文字受源图分辨率限制，1px 级边缘差无法消除。
3. **比例不一致**。源图宽高比与页面宽高比不同时，纵向内容高度会小于页高，
   内容区之外的差异不算错。
4. **笔画粗细**用面积比核，不要看 IoU。IoU 对 1px 边缘位移很敏感，
   而面积比才反映粗细（见 Q14）。

### Q21：色带整条不见了，字标变成黑字压在插图上

典型症状：导出图里垫底色带消失、反白字标变成黑字且位置错乱，
但定位日志仍然打印"误差 0.000mm [OK]"。

原因是 **`Layer.Import` 把新形状插到图层"底部"（索引 1），不是追加到末尾**。
如果垫底矩形和反白字标在**同一个图层**，导入后取 `Item(Count)` 会拿到矩形，
于是把字标的尺寸/位置/填充写到了矩形上——两个形状属性互换。

实测对照：

| 形状 | 应该是 | 实际变成 |
| --- | --- | --- |
| `band01`（矩形） | 210 × 27.264 mm，深色 `#121011` | 70.856 × 19.801 mm，白色 |
| `band01_ink`（字标） | 70.856 × 19.801 mm，白色 | 70.852 × 19.775 mm，深色 `#111111` |

日志之所以报 OK，是因为它打印的是**请求值**而不是读回的实际值。

**修法**（技能内已修）：一个区域一个图层，让导入时图层为空；
定位新形状改用"导入前后名称集合的差"；矩形也读回 `PositionX/SizeWidth` 核对。

> 手工配置里矩形和字标天然在不同图层，所以这个 bug 只在自动分区把两者
> 归到同一图层时才暴露。

### Q22：报告里的还原度数字对不上（IoU 显示 24%，实际看着挺好）

报告里的还原度来自**配准校验阶段**，而写报告曾经排在它**之前**，
于是报告写的是校验前的空值/旧值。实测症状：`report.md` 里
`整页 IoU 24.47%`、`召回 37.31%`，而 `compare\metrics.json` 里是
`93.62% / 95.14%`。

两件事都修了：

1. 写报告移到校验**之后**（顺序问题）；
2. 清单 `manifest.json` 补记每区的 `iou/recall/precision/ink_ratio`、
   垫底矩形 `rects`、图层顺序 `layer_order`，报告可脱离描摹独立重建：

```powershell
python scripts\cdr_bitmap_to_cdr.py --report-only ^
  --out-dir out\cover_work --output out\cover.cdr
```

几秒钟出新报告，不必重跑十几分钟描摹。细节见「十、报告为什么可以单独重建」。

### Q23：转活字时，把一块**插图**当成了文字，还在 CDR 里建了一行字

真实踩过的坑。反向测试时把工具插图区当"文字区"喂进 `cdr_text_live.py`：

```
OCR 幻觉文本 : 'wander'（置信 0.731）
最佳字体     : Impact Regular   IoU 0.3859   自然宽度比 1.887
lift         : 1.531
判定         : convert          ← 错！
结果         : 在 CDR 里建了一行 'wander'
```

**为什么两条"相似度"判据全过**：`lift` 与 IoU 都是在
**"按高度归一 + 横向拉伸到同宽"之后**算的。对非文字区域，横向拉伸这一步
本身就把最大的差异（宽高比完全不符）抹掉了，剩下的分数是拉伸出来的假象。
绝对 IoU 通道没过（0.3859 < 0.72），但相对通道全过（1.531 > 1.25、0.3859 > 0.35）。

**修法**：补两条**与字号无关的比值**当硬门槛，两条都过才允许转：

| 判据 | 含义 | 真页脚实测 | 插图误判区实测 |
| --- | --- | --- | --- |
| 连通分量数 ÷ 字符数 | 一个字形一个连通分量，比值应 ≈ 1 | **1.10** | **11.00** |
| 最佳候选的自然宽度比 | 字体的自然字宽应与源宽度相符 | **0.992** | **1.887** |

这两条能用**绝对区间**（不像 IoU 只能比相对值），因为它们是**比值**：
期望值恒为 1，与字号无关，不会随字变小而退化。

它们还**互补**——这是能一起用住的原因：

- **短幻觉**（几个字盖住复杂图形）→ 分量数÷字符数抓到（比值被抬高）
- **长幻觉**（长串盖住简单图形）→ 自然宽度比抓到（自然宽度远大于源宽度）

修完反向测试连 CDR 都不再生成，正向的页脚判定与数字**零变化**
（IoU 0.7235、宽度比 0.992、lift 1.68、分量/字符 1.10）。

产物里同时给出 `reject_kind` 区分两类拒绝，处置方式不同：

- `not_text` —— 这块**根本不是文字**，该去查区域切分；
- `weak_match` —— 是文字但库里没有够接近的，只能保留描摹轮廓。

**残余风险**：两条门槛只覆盖两个维度。若某个非文字区域恰好同时满足
（分量数与幻觉出的字符数相称、宽高比也与某字体的自然字宽相符），仍可能被放过。
定阈值时只有**一个**负样本，所以宁可把区间放宽——错拦会把真文字降级，
比漏拦更糟。这也是 `--apply` **始终把活字建在单独图层**的原因：
猜错了整层删掉即可。

### Q24：日志打印了"已保存"，但重新打开文件发现什么都没变

**这是最阴的一种失败**，因为它**不报错**。

现象：`--apply` 整轮跑完、内存里的回读核验全对、日志打印"已保存 xxx.cdr"，
但重新打开文件一看——描摹层还在、活字没有。白跑一轮。

**原因**：目标 CDR 正被另一个程序打开（最常见就是**你自己开着 CorelDRAW
在看这个文件**）。这时文件是只读的，而 `doc.Save()` 在这种情况下
**既不抛异常、也不写文件**：

| 现象 | 实际情况 |
| --- | --- |
| `doc.Save()` 返回 | 正常，无异常 |
| 日志 | 照常打印"已保存 \<路径\>" |
| 文件 mtime / 大小 | **完全没变** |
| 内存里的文档 | 改动都在 → 所以"回读核验"**全对** |

核验对象选错了：核验的是**内存**，应该核验**磁盘**。

**处理**：脚本现在会在保存前后各取一次 `(大小, mtime_ns)` 指纹比对，
没变就报错并返回**退出码 3**。看到 3 就按提示**关掉那个文件再跑一遍**。

> **⚠️ 但占用往往是自动化自己造成的，别急着怪用户。**
> `cdr_text_live.py --apply` 走 `open_document()`，刻意"只开不关"
> （把成果留在窗口里让你接着改）——于是**下一轮**重建同一个文件时，
> 目标就被上一轮留下的文档占着。所以流水线现在会在归档旧文件之前、
> `cdr_image_place.py` 会在 `SaveAs` 之前，都先调
> `cdr_common.release_document()` 把目标关掉（只关路径匹配的那一个，
> `Dirty=True` 时拒绝关闭）。详见 Q27。

**排查顺序**（比猜快得多）：

1. 做**对照写测试**，区分"文件被占用"和"整个目录被拦"：
   往同目录新建一个文件、再试同目录里**别的** `.cdr`。
   都能写、只有目标不行 → 就是**这个文件被独占**。
2. 找出谁占着它。手动启动的 CorelDRAW **不在 ROT 里**，
   `GetActiveObject` 会报 `操作无法使用 (-2147221021)`；
   但 `Dispatch('CorelDRAW.Application.18')` **往往能附加上去**，
   这时 `app.Documents` 就能列出它开着哪些文档。
3. 关掉那个文档：先读 `doc.Dirty`，**为 True 就别动**（有未保存内容）；
   为 False 才 `doc.Close()`。

> **一般化教训：`调用没报错` ≠ `结果发生了`。**
> 凡是跨进程落盘的写文件，都要**回读磁盘**确认。

> **另一条：不要为了"让文件存在性检查有意义"而先删旧文件。**
> 受限运行环境会把 `os.remove` 判为批量删除并**直接掐掉进程**，
> 日志里只剩一行 `SAFE_DELETE_BULK_CONFIRM_REQUIRED`，
> 整条流水线莫名 `EXIT=1`，而 CDR 其实已经保存好了——极易误判成保存失败。
> 用**指纹比对**或**改名归档**替代删除。

### Q25：源图里第一个字母是大写 `V`，活字却建成了小写 `v`

**真踩过。** 页脚源图是 `O.V.D. Importadora …`，活字建出来是 `O.v.D. …`。
逐条查下来，是**三个叠在一起的缺陷**，而且前两个都**不报错**：

#### 缺陷一：根本没有"上下同形字母大小写"的判据

`o/O`、`v/V`、`c/C`、`s/S`、`w/W`、`x/X`、`z/Z` 这几组字母**形状本来就一样、
只差高度**，所以 OCR 认错时没有任何兜底。修法是加 `case_by_height()`，
按"行内字高"裁决（原理见上一节）。

#### 缺陷二：`_col_runs()` 收到灰度数组，静默把整词当成一个字形

这两个函数是按**布尔掩膜**写的，`ndarray.any(axis=0)` 对布尔数组是
"这一列有没有墨"，对 uint8 灰度数组却是"这一列有没有**非零值**"——
背景灰度约 240 全部非零，于是整个词被判成**一个**列段：

```
'O.v.D.'  字母数 3   段数 1   [16]     ← 灰度传进去的样子（高 16 = 整幅裁切高度）
'O.v.D.'  字母数 3   段数 5   [13,2,12,12,2]  ← 正确
```

不报错、不抛异常，只是把"逐字切分"退化成"整词一块"，后续所有字高判据一起失效。
修法是加一层 `_as_mask()` 归一化。

#### 缺陷三：调用方把**整串文本**当词列表传了进去

```python
hprops, hnotes = case_by_height(core, words, text)   # ← 第三个参数应该是 toks
```

`len('O.v.D. …')` = 60，`len(words)` = 9，命中函数开头"词数不一致 → 放弃"的守卫，
**静默返回空建议**。整条修复等于没接线，日志上什么都看不出来。

修法：函数内对 `str` 入参**直接抛 `TypeError`**。
**不要"顺手 split 一下"把调用方的错掩盖掉**——那只会让下一个同类错误继续沉默。

> **一般化教训：参数类型不对时，函数必须报错，不能安静地不干活。**
> 上面两条（灰度当布尔、字符串当列表）是同一类错误：调用成功、返回正常、
> 结果为空，只有输出对不上时才发现。凡是"某种输入下会静默返回空"的函数，
> 都该先问一句：这个空是真的没意见，还是参数根本没对上？

#### 判定依据：两条独立几何证据

| 证据 | 源图实测 | 大写 `V` | 小写 `v` | 结论 |
| --- | --- | --- | --- | --- |
| 该字**高度** | 12 px | 12~13 px（cap） | 9 px（x-height） | 大写 |
| 该字**宽度** | 8 px | 8 px | 6~7 px | 大写 |

#### 为什么像素 IoU 帮不上忙

同一字体下只改这一个字母的大小写：

| 文本 | 整行 IoU | 逐词对齐 IoU |
| --- | --- | --- |
| `O.v.D.`（错） | **0.7244** ← 错的反而高 | 0.7056 |
| `O.V.D.`（对） | 0.5090 ← 对的反而低 | **0.7235** |

38×13 px 的词里，单字形的大小写差异已在 IoU 类指标的**噪声底**附近。
整行口径还会**把方向搞反**（因为它被"横向拉伸的相位"主导，见 Q26），
所以连"用整页 IoU 验证一下"这条路也走不通——**文本正确性以几何证据为准**。

#### 顺带发现的坑：那个看不见的句点会污染"字形宽度"

`O.V.D.` 里 V 与 D 之间那个句点在二值图上几乎消失，只在 V 右边留下
3 个杂散像素（x21~23）。它们被并进 V 的列段，于是 V 的"列段宽度"量出来是
**10px**，而它真正的笔画只有 **8px**。

**所以判据只能用高度、不能用宽度**：杂散像素只有 1~2 行高、落在基线附近，
抬不动 cap-height 那一端的跨度，但会把宽度撑大。

#### 怎么确认修好了

看 `--apply` 的日志里有没有这一行，以及 `live_text.json` 里的 `case_fixes`：

```
形状修正建议: …; v->V(高12: x9/cap12)
逐词裁决: …; O.v.D.->O.V.D. ✅
```

`高12: x9/cap12` = "这个字实测高 12px，本行 x-height 9px、cap-height 12px"，
可以直接拿去和源图核对。若**没**出现这一行，说明锚点不足或该行区分度不够
（见上一节的安全约束）——那就得人工看一眼，别硬改。

### Q26：改对了文本，选出来的字体反而更差了

**真踩过，而且是被上一条修复"带出来"的。** 把页脚文本从 `O.v.D.` 改成
`O.V.D.` 之后，最佳字体从 `Swis721 Cn BT / Bold` 变成了
`Swis721 BlkCn BT / Black`——笔画明显更重，肉眼能看出偏粗。

**原因**：字体匹配的相似度用的是**整行 IoU**：把渲染结果按高度归一后
**横向拉伸到与源同宽**，再逐像素比。这一步会让每个字形的横向位置误差
**累积**（行末能到几像素），而小字号下笔画只有 1~2px 宽——于是这个分
基本由"拉伸的相位"决定，而不是由字形决定。实测**同一字体**只改一个
字母的大小写：

| 文本 | 整行 IoU | 逐词对齐 IoU |
| --- | --- | --- |
| `O.v.D.`（错） | **0.7244** ← 错的反而高 | 0.7056 |
| `O.V.D.`（对） | 0.5090 ← 对的反而低 | **0.7235** |

整行口径**把方向搞反了**，于是把笔画更重的 Black 排到了前面。

**修法**：改成**逐词对齐**——先按列投影切词，再逐词紧裁后比（`_word_aligned_iou`）。
方向就正过来了，而且与一条独立证据一致——**墨迹密度**：

| | 墨迹密度 |
| --- | --- |
| 源图 | 0.2852 |
| `Swis721 Cn BT / Bold` | **0.3106** ← 最接近 |
| `Swis721 BlkCn BT / Black` | 0.3563 |

**代价与边界**：逐词拉伸抹掉了词内字距差异，所以**词间宽度差必须继续由
`width_ratio` 单独把关**（它算的是整行自然宽度比，不经拉伸，仍是硬门槛）。
词数切不一致时该指标返回 `None`，自动回退到整行 IoU。

#### ⚠️ 换度量必须连门槛一起重校

这是本次最容易漏的一步。同一页脚：

| | 旧口径 | 新口径 |
| --- | --- | --- |
| `final_iou` | 0.7244 | 0.7235（几乎没动） |
| `lift`（领先中位倍数） | 2.675 | **1.68**（掉了一半） |

中位候选的 IoU 也一起抬高了，所以相对优势本来就比整行口径下小。
**只改度量不改门槛，一个本该转活字的页脚会被"相似度不足"拒掉**（真踩过）。

还有一处**同源**要求：重建复核 `rebuild_iou` 必须和 `match_fonts` 用**同一个口径**。
当时排名已经换成逐词对齐、`rebuild_iou` 却还留着整行，于是 `final_iou` 与
`median_iou` 来自两套度量，`lift` 被算成 1.18（真实是 1.68），页脚被误拒。

> **一般化教训：一批数字必须同源；换度量就要把阈值、中位数、复核路径一起重算。**
> 混用两套口径算出来的比值是废数，而且它看起来完全正常。

### Q27：重建时报 `PermissionError [WinError 32] 另一个程序正在使用此文件`

**这不是偶发故障，是循环迭代的必然结果。**

`cdr_text_live.py --apply` 走 `cdr_common.open_document()`，它**刻意"只开不关"**
——把成果留在 CorelDRAW 窗口里让你接着改。于是**下一轮**重建同一个文件时，
目标就被上一轮留下的文档占着：

- 改名/删除直接抛 `PermissionError [WinError 32]`（就是这条）；
- 更阴的是 `SaveAs` / `Save()` 会**静默变成空操作**：不抛异常、不写盘，
  日志照常打印"已保存"，只有磁盘没变。整轮跑完、回读内存核验还全对。

**现在的处理**（不用你手动干预）：

- `cdr_image_place.py` 在 `SaveAs` 之前、流水线在归档旧文件之前，
  都会先调 `cdr_common.release_document(path)` 解除占用；
- 该函数只关**路径匹配**的那一个文档（按绝对路径规范化比对，
  同名不同目录不会误伤）；
- **`Dirty=True` 时拒绝关闭**——那是你的改动，脚本不会替你丢；
- 释放失败会明确报错退出，不再静默往下跑。

如果你看到"目标文件被占用且无法自动释放"，说明那个文档有未保存改动。
自己看一眼、决定存还是弃，然后重跑即可。

### Q28：横向稿画出来右侧溢出页面（纵向稿却没事）

**页面方向被写死了。** `Page.SetSize(w, h)` 本身就会按宽高把方向设对
（宽 > 高 → 横向），但如果后面又无条件写一句方向，就会把刚设好的宽高**交换掉**：

```python
page.SetSize(page_w, page_h)
page.Orientation = CDR_PORTRAIT     # ← 0，纵向。横向稿被交换成竖向！
```

实测后果：210 × 167.16 mm 的横向稿，页面变成 **167.16 × 210**，
而内容仍按 210 mm 宽排布 → **右侧 43 mm 溢出页面**。

**这个 bug 藏了很久**，因为另一张 210 × 286.4 的**纵向**稿恰好是对的，
所以只测纵向稿发现不了。修法是方向必须由宽高推导：

```python
page.Orientation = CDR_LANDSCAPE if page_w > page_h else CDR_PORTRAIT
```

X8 实测枚举：`cdrPortrait = 0`、`cdrLandscape = 1`。

> **交付前一定回读 `SizeWidth / SizeHeight / Orientation` 确认**，
> 别只信自己请求的值。`SetSize` 会自作主张，后面那句赋值也会。

---

## 八、脚本参数速查

### cdr_scan_text.py（整图识别）

| 参数 | 说明 |
| --- | --- |
| `--image` | 源位图路径（必填） |
| `--out-dir` | 产出目录（`scan.json`、`scan_preview.png`） |
| `--mm-per-px` | 标定比例，与后续步骤必须一致 |
| `--emit-live` | 导出活字区域清单（`NAME=y0,y1,x0,x1[,#RRGGBB][,rot=180]`） |
| `--emit-trace` | 导出描摹区域清单（`name:x0,y0,x1,y1[,#RRGGBB]`） |
| `--min-text-iou` | 文字判定的字形 IoU 门槛 |
| `--min-block-ink` | 图形块的最小墨迹量门槛 |
| `--group-gap` | 块聚类的间隙（px） |

### cdr_bitmap_to_cdr.py（主入口）

| 参数 | 说明 |
| --- | --- |
| `--image` | 源位图路径（PNG/JPG）；`--report-only` 时可省略 |
| `--output` | 输出 CDR 路径；`--report-only` 时可省略 |
| `--page-width` | 页面宽度（毫米） |
| `--page-height` | 页面高度（毫米）；省略则按图比例推导 |
| `--page-size` | 标准纸型 `A4`/`A3`/`A5`/`Letter` 或 `宽x高`；给定时覆盖上面两项的高度 |
| `--region` | 手工分区 `名称:x0,y0,x1,y1[,invert]`，可重复；给出后不再自动分区 |
| `--rect` | 手工追加垫底矩形 `名称:x,y,w,h,#RRGGBB`（毫米，y 距页顶） |
| `--out-dir` | 中间产物与报告目录；默认 `<CDR 同目录>/<文件名>_work` |
| `--upscale` | 描摹前灰度上采样倍数，默认 **8** |
| `--turdsize` | 斑点面积阈值（源图像素），默认 2 |
| `--alphamax` | 拐角阈值，默认 1.0；**不要用 0** |
| `--opttolerance` | 曲线优化容差，默认 0.1 |
| `--threshold` | 固定二值化阈值；给定时不做逐区寻优 |
| `--tune-thresholds` | 寻优候选阈值，默认 `112,118,128,138,148` |
| `--max-tune-px` | 源像素面积超过此值的区域只扫 3 个候选阈值，默认 200000 |
| `--crop-left` | 忽略源图最左侧 N 列（裁切/扫描残留），并按其后的宽度重新标定 |
| `--no-split` | 不做列方向切分 |
| `--trace-only` | 只描摹出 SVG 与清单，不建 CDR（无 CorelDRAW 也能跑） |
| `--no-validate` | 跳过配准式像素校验 |
| `--report-only` | 不重跑描摹与建 CDR，仅用已有清单与指标重建报告 |
| `--keep-existing` | 不自动关闭同名已打开文档 |
| `--progid` | CorelDRAW ProgID，默认 `CorelDRAW.Application.18` |

`--report-only` 时 `--image` / `--output` 可省略，但需要 `--out-dir`
（或用 `--output` 推导出工作目录）。它的存在理由见下节「十、报告为什么可以单独重建」。

### cdr_prompt_builder.py

| 参数 | 说明 |
| --- | --- |
| `--source` | 必填，源 CDR 路径 |
| `--output` | 输出 Markdown 提示词路径 |
| `--progid` | CorelDRAW ProgID，默认 `CorelDRAW.Application` |
| `--invisible` | 以不可见方式运行 |

### cdr_redraw.py

| 参数 | 说明 |
| --- | --- |
| `--source` | 必填，源 CDR 路径 |
| `--output` | 输出 CDR 路径，默认 `outputs/redraw.cdr` |
| `--mode` | `clone`（形状级重建，默认）/ `duplicate`（文件级复制） |
| `--progid` | CorelDRAW ProgID |
| `--invisible` | 以不可见方式运行 |
| `--keep-source-open` | 结束后保持源文档打开 |
| `--report` | 输出 JSON 校验报告路径 |

> 输出路径已存在时，脚本会自动追加时间戳，**绝不覆盖**已有文件。

### cdr_image_trace.py

| 参数 | 说明 |
| --- | --- |
| `--image` | 必填，参考图路径（PNG/JPG） |
| `--page-width` | 必填，页面宽度（毫米），用于标定 mm/px |
| `--page-height` | 页面高度（毫米）；省略则按图比例推导 |
| `--region` | 区域定义 `名称:x0,y0,x1,y1[,invert]`，可重复；坐标是源图像素 |
| `--auto` | 自动按投影分割区域 |
| `--out` | 输出目录，默认 `svg` |
| `--upscale` | 描摹前灰度上采样倍数，默认 **8**（细部保真的关键；超大区域可用 4 控速） |
| `--turdsize` | 斑点面积阈值（源图像素），默认 2；内部按 `upscale²` 折算 |
| `--alphamax` | 拐角阈值，默认 1.0；**0 = 纯多边形，不要用** |
| `--opttolerance` | 曲线优化容差，默认 0.1 |
| `--threshold` | 二值化阈值，默认 128。**越低笔画越细**；细笔画文字建议用 `--probe` 扫 110~150 |
| `--probe` | 参数扫描模式，只输出候选参数 IoU，不写 SVG |
| `--crop-left` | 忽略源图最左侧 N 列（裁切/扫描残留） |

### cdr_image_place.py

| 参数 | 说明 |
| --- | --- |
| `--manifest` | 必填，`cdr_image_trace.py` 产出的 `manifest.json` |
| `--output` | 必填，输出 CDR 路径 |
| `--preview` | 预览 PNG 路径；省略则不导出 |
| `--progid` | CorelDRAW ProgID，默认 `CorelDRAW.Application.18` |
| `--rect` | 附加原生矢量矩形 `名称:x,y,w,h,#RRGGBB`，可重复（y 为距页顶毫米） |
| `--rect-layer` | 承载 `--rect` 矩形的图层名，默认 `RECT`。**配合 `--layer-order` 时该名必须出现在 order 里**，否则矩形无处安放会报 KeyError |
| `--layer` | 图层映射 `区域名=图层名`，可重复 |
| `--white` | 需要填白的区域名（深底上的反白元素），可重复 |
| `--layer-order` | 图层自下而上的顺序，逗号分隔 |
| `--close-existing` | 构建前关闭名称以这些前缀开头的文档，逗号分隔 |

### cdr_visual_diff.py

| 参数 | 说明 |
| --- | --- |
| `--source` | 必填，源参考图路径 |
| `--render` | 必填，CDR 导出的预览图路径 |
| `--placement` | 必填，`cdr_image_place.py` 产出的 `placement.json` |
| `--out` | 输出目录，默认 `compare` |
| `--width` | 比对用整页位图宽度，默认 2480（约 300dpi A4） |
| `--threshold` | 二值化阈值，默认 128 |

### cdr_text_live.py（文字转活字，可选）

| 参数 | 说明 |
| --- | --- |
| `--image` | 必填，源位图 |
| `--region` | 必填，文字区 `名称=y0,y1,x0,x1`（源图像素，左上原点），可重复 |
| `--mm-per-px` | 必填，标定比例 |
| `--out-dir` | 必填，产物目录 |
| `--min-iou` | 相似度绝对通道，默认 **0.72**（适合大字号） |
| `--min-lift` | 相似度相对通道，默认 **1.25**（与字号无关） |
| `--min-iou-floor` | 相对通道下限，默认 **0.35** |
| `--min-width-ratio` | **硬门槛**：自然宽度比下限，默认 **0.65** |
| `--max-width-ratio` | **硬门槛**：自然宽度比上限，默认 **1.50** |
| `--min-cc-per-char` | **硬门槛**：连通分量数÷字符数 下限，默认 **0.25** |
| `--max-cc-per-char` | **硬门槛**：连通分量数÷字符数 上限，默认 **4.0** |
| `--corel-only` | 只用 CorelDRAW 字体表里有的字体（需要 CDR 可用） |
| `--apply` | 把判定为 `convert` 的区域真的建成活字并写入这个 CDR；文件不存在则按源图尺寸新建 |
| `--replace-traced` | `REGION=LAYER`，建好活字后删掉该图层的描摹轮廓，可重复。**不是可选项**，见「六、文字转活字」 |
| `--live-layer-suffix` | 活字图层后缀，默认 **`_LIVE`**（`footer` → `footer_LIVE`） |
| `--live-color` | 活字填充色，默认 `#000000` |

判定的完整逻辑见「六、文字转活字」与 Q23；保存失效见 Q24。

---

## 九、退出码

| 退出码 | 含义 |
| --- | --- |
| `0` | 校验通过 |
| `2` | 校验存在差异（CHECK WARNINGS） |
| 其他 | 运行失败，异常信息在 stderr |

适合接入 CI 或批处理脚本。

主入口 `cdr_bitmap_to_cdr.py`：`0` = 成功（建出 CDR，或 `--trace-only` 下描摹成功）；
`1` = 输入有误（找不到源图、没给页面尺寸）；`2` = 描摹成功但没建出 CDR
（CorelDRAW 不可用或保存失败）。**描摹段产物与报告在 `2` 时仍然有效**，
可以直接用 `--trace-only` 重跑或单独调用 `cdr_image_place.py` 补建。

`cdr_text_live.py --apply`：`0` = 活字建好**且已确认写进文件**；
**`3` = 活字建好了但保存没有落盘**（见 Q24）——文件被别的程序占用着，
关掉它再跑一遍即可。

---

## 十、报告为什么可以单独重建

报告里的数字来自流水线的**两个不同阶段**：

| 阶段 | 产出 | 报告里的位置 |
| --- | --- | --- |
| 步骤 4 逐区寻优 + 描摹 | 每区的阈值、IoU、面积比 | 「分区与参数」表 |
| 附加 配准式像素校验 | 整页/分区域 IoU、召回、精确 | 「还原度」表 |

早先的实现把写报告放在校验**之前**，于是报告里的还原度停留在有 bug 时期的
旧值——整页 IoU 显示 **24.47%**，而真实值是 **93.62%**。顺序修好后还有第二个问题：
报告措辞改一个字也要重跑十几分钟描摹，不可接受。

所以清单 `manifest.json` 里补齐了报告所需的全部字段：

```json
{
  "source_image": "...", "page_mm": [210.0, 286.4421], "mm_per_px": 0.17255546,
  "image_px": [1217, 1660], "params": {"upscale": 8, "alphamax": 1.0},
  "page_h_explicit": false,
  "layer_order": ["BAND", "02_INK", "03_MARK", "04_ART", "05_TEXT"],
  "rects": [{"name": "band01", "x": 0, "y": 155.47, "w": 210, "h": 27.26,
             "rgb": [18, 16, 17]}],
  "regions": {
    "band01_ink": {
      "svg": "...", "bbox_mm": [...], "size_mm": [...], "invert": true,
      "source_box_px": [0, 901, 1217, 1059], "subpaths": 13,
      "type": "BAND", "threshold": 138, "kind": "band", "color": [18, 16, 17],
      "iou": 99.51, "recall": 99.76, "precision": 99.75, "ink_ratio": 1.0
    }
  }
}
```

有了这些，报告就能完全脱离描摹过程重建：

```powershell
python scripts\cdr_bitmap_to_cdr.py --report-only ^
  --out-dir out\cover_work --output out\cover.cdr
```

它会读 `svg\manifest.json` 与 `compare\metrics.json`，不必打开源位图
（源图被移动或删除也不影响），几秒钟出报告。

两个细节：

- **缺字段显示 `—`，不显示 0。** 旧清单没有 `iou` 时若按 0 打印，
  会被误读成「完全不像」；按 `—` 打印才是诚实的。
- **垫底矩形能反推。** 若清单里没有 `rects`（旧版清单），
  满版色带的垫底矩形可由该区域的 `bbox_mm` 直接推出，报告不会缺这一段。
